...
export default async function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  // Panggil fungsi yang sudah kita pisahkan
      ...
      <div className="p-8 bg-white rounded-lg shadow-md">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          ...
          <div className="pt-4">
            ...
            <p className="mt-4 text-gray-600">{product.description || `Deskripsi singkat tentang ${product.name}.`}</p>
          </div>
        </div>
      </div>
        ...
        <div className="mt-6 space-y-6">
          {product.reviews.length > 0 ? (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center">
                      {[...Array(5)].map((_, i) => <StarIcon key={i} filled={i < calculateAverageRating(review)} />)}
                    </div>
                  </div>
                </div>
            ...
            <div className="p-8 text-center bg-white border rounded-lg">
                <p className="text-gray-500">Belum ada ulasan untuk produk ini. Jadilah yang pertama memberi ulasan!</p>
            </div>
          )}
        </div>
      </div>
