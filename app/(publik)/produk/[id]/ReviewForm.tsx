...
// Komponen ikon bintang interaktif
...
const RatingRow = ({ label, rating, setRating }: { label: string, rating: number, setRating: (r: number) => void }) => {
    const [hoverRating, setHoverRating] = useState(0);
    return (
        <div className="flex items-center justify-between py-2"><p className="text-gray-700">{label}</p><div className="flex" onMouseLeave={() => setHoverRating(0)}>{[1, 2, 3, 4, 5].map(star => (<StarIcon key={star} filled={(hoverRating || rating) >= star} onHover={() => setHoverRating(star)} onClick={() => setRating(star)}/>))}</div></div>
    );
};
...
export default function ReviewForm({ productId }: { productId: number }) {
  ...
  const { isLoading, showLoading, hideLoading } = useLoading(); // Ambil isLoading dari context
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch('/api/reviews', {
        ...
        body: JSON.stringify({ ...ratings, comment, productId, reviewerName }),
      });
      ...
      if (res.ok) {
        ...
        setReviewerName('');
        router.refresh();
      } else { setError(data.message || 'Gagal mengirim ulasan.'); }
    } catch (err) {
    ...
    <div className="p-6 mt-8 bg-white border rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit} className="mt-4 space-y-2 divide-y">
        ...
        <div className="pt-4">
          ...
          <input type="text" value={reviewerName} onChange={(e) => setReviewerName(e.target.value)} className="w-full px-4 py-2 mt-1 border rounded-lg" placeholder="Masukkan nama Anda" required />
        </div>
        <div className="pt-4">
        ...
        <div className="pt-4 text-right">
          {/* PERBAIKAN: Gunakan 'isLoading' untuk kondisi disabled */}
